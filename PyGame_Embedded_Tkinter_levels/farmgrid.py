...
class FarmGrid:
    ...
    def generate_plain(self):
        ...
        self.grid = [
            [FarmTile(x, y, 0) for y in range(self.height)] for x in range(self.width)
        ]
        ...
        for y in range(self.height):
            for x in range(self.width):
                if river_orientation == 0:
                    ...
                    if x == int(
                        y * (river_start - river_end) / (self.width) + river_start
                    ):
                        self.grid[x][y] = FarmTile(x, y, 2)  # water
    ...
    def generate_crops(self):
        ...
        avail_positions = [
            ...
            for y in range(self.height)
            if ((x, y) != (0, 0))
        ]  # possible positions except farmer initial position

        crop_count = int(
            self.width * self.height * 0.2
        )  # 20% of the grid will have crops on it
        random.shuffle(
            avail_positions
        ...
        for _ in range(crop_count):
            if avail_positions:
                ...
                x, y = (
                    avail_positions.pop()
                )  # pop from list and get random available position
