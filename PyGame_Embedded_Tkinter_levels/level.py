...
class Levels:
    def __init__(self): # initialises levels dictionary with data and sets current level to lvl 1
        self.levels = {
            1: {
                ...
                'config': 'plain',
                'requires_multiple_test_cases': False, # set to false if code is to be run on only one farm instance
                'unlocked': True
            },
    ...
    def load_progress(self):
        # Load the saved progress from JSON file
        try:
            ...
            # File does not exist, initialize to default
    ...
    def get_test_cases(self): # returns true if level requires multiple test cases
    ...
    def check_current_level_completion(self, farm_stats): #generic func that passes level specific check function
