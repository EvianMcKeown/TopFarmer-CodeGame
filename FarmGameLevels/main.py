...
def start_level(level_index):
    ...
    task = levels[level_index]["task"]
    show_task_window(task, lambda root: root.destroy())

def execute_python_code(code):
    code = code.replace('farmer.', 'farm.farmer.')  # prevents user needing to type farm.farmer each time
    try:
        exec(code, {'farm': farm, 'print': print}, globals())
        if levels[current_level]["check"](farm):
            show_congratulations_window(next_level)
        else:
            show_failure_window(retry_level)
...
def handle_button_run():
    ...
    if user_input:
        # split by lines '\n', so that every line is shown as it is executed
...
# Main loop
running = True
start_level(0) #start at first level
play_music("theme")
while running:
