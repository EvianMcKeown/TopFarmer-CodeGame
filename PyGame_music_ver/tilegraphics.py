...
class TileGraphics:
    ...
    def load_graphics(self):
        ...
        for i in range(2):  #2 frames in the sheet
            frame = self.farmer_sprite_sheet.subsurface((i * 48, 0, 48, 48))  # Cut images from sheet
            scaled_frame = pygame.transform.scale(frame, self.farmer_scale)  # Scale farmer frames
        ...
        for key in self.crop_images:
            self.crop_images[key] = pygame.transform.scale(self.crop_images[key], (self.scale_factor, self.scale_factor))

    def get_farmer_frame(self):
        # Update farmerframe based on time
        current_time = pygame.time.get_ticks()
        if current_time - self.last_update_time > self.frame_delay:
            self.farmer_frame_index = (self.farmer_frame_index+1) % len(self.farmer_sprites)
    ...
    def render_farmer(self, surface, x, y):
        ...
        surface.blit(farmer_frame, (x * self.scale_factor + offset_x, y * self.scale_factor + offset_y))
